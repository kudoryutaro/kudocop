...
if __name__ == '__main__':
    ...
    assert args.target_atom_type is not None, 'set --target_atom_type or -t'

    print(f'target_atom_type : {args.target_atom_type}')
    ...
    if args.file_name is None:
        ...
        df_coordination_number_distribution = sdats.get_coordination_number_distribution(target_atom_type=args.target_atom_type, bond_type=args.bond_type, cut_off=args.cut_off,
                                                                                         condition=condition)
        ...
        print(df_coordination_number_distribution)
    else:
        # SimulationDat
        sdat = SimulationDat()
        ...
        coordination_number_distribution = sdat.get_coordination_number_distribution(
            target_atom_type=args.target_atom_type, bond_type=args.bond_type, cut_off=args.cut_off, condition=condition)
        print(f'bond_type: {args.bond_type}, cut_off: {args.cut_off}')
        print(f'condition: {args.x_min} <= x <= {args.x_max}')
        print(f'condition: {args.y_min} <= y <= {args.y_max}')
        print(f'condition: {args.z_min} <= z <= {args.z_max}')
        print(coordination_number_distribution)
